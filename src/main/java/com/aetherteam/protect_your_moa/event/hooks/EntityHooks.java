...
public class EntityHooks {
    public static Optional<InteractionResult> interactionOpenMoaInventory(Entity target, Player player) {
        if (target instanceof Moa moa && moa.isPlayerGrown() && !moa.isBaby() && player.isShiftKeyDown() && player.getMainHandItem().isEmpty()) {
            if (player instanceof ServerPlayer serverPlayer) {
                moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).openInventory(serverPlayer, moa);
            }
    ...
    public static Optional<InteractionResult> equipMoaArmor(Entity target, Player player, ItemStack stack) {
        if (target instanceof Moa moa && moa.isPlayerGrown() && !moa.isBaby() && !moa.isVehicle()) {
            if (!stack.isEmpty() && stack.getItem() instanceof MoaArmorItem && moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).isArmor(stack) && !player.isSecondaryUseActive() && !moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).isWearingArmor()) {
                if (!player.level().isClientSide()) {
                    moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).setSynched(moa.getId(), INBTSynchable.Direction.CLIENT, "equipArmor", stack);
    ...
    public static Optional<InteractionResult> equipMoaChest(Entity target, Player player, ItemStack stack) {
        if (target instanceof Moa moa && moa.isPlayerGrown() && !moa.isBaby() && !moa.isVehicle()) {
            if (!stack.isEmpty() && stack.is(Items.CHEST) && !player.isSecondaryUseActive() && !moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).hasChest()) {
                if (!player.level().isClientSide()) {
                    moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).setSynched(moa.getId(), INBTSynchable.Direction.CLIENT, "setChest", true);
                    moa.playSound(ProtectSoundEvents.ENTITY_MOA_CHEST.get(), 1.0F, (moa.getRandom().nextFloat() - moa.getRandom().nextFloat()) * 0.2F + 1.0F);
                    if (!player.getAbilities().instabuild) {
                        stack.shrink(1);
                    }
                    moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).createInventory();
                    return Optional.of(InteractionResult.CONSUME);
                } else {
                    return Optional.of(InteractionResult.SUCCESS);
                }
    ...
    public static void onJoinLevel(Entity entity) {
        if (entity instanceof Moa moa) {
            moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).onJoinLevel();
        }
    }

    public static void onUpdate(Entity entity) {
        if (entity instanceof Moa moa) {
            moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).onUpdate();
        }
    }

    public static void onDrops(Entity entity, Collection<ItemEntity> drops) {
        if (entity instanceof Moa moa) {
            Container inventory = moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).getInventory();
            if (moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).hasChest()) {
                ...
                moa.getAttachedOrCreate(ProtectDataAttachments.MOA_ARMOR).setChest(false);
            }
            if (inventory != null) {
